...
describe('Create utility script command', () => {
  ...
  it('should create the command for saint_domain_enrich', () => {
    ...
    const expected = 'docker run --rm -v $(pwd):/files/ pvutilitiespython /app/saint_domain_enrich.py '
  ...
  it('should create the command for saint_fea', () => {
    ...
    const expected = 'docker run --rm -v $(pwd):/files/ pvutilitiespython /app/saint_fea.py -f 0.01 -s file.txt -t 0';
    expect(createCommand(fields)).toBe(expected);
  });
  ...
  it('should create the command for saint_stats', () => {
    ...
    const expected = 'docker run --rm -v $(pwd):/files/ pvutilitiespython /app/saint_stats.py -f 0.01 -s file.txt';
    expect(createCommand(fields)).toBe(expected);
  });
});
